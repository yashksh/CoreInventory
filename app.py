...
def _seed_master_data():
    """Seed warehouses, locations, contacts, products, inventory."""
    # ── Warehouses ──
    wh_data = [
    ...
    db.session.commit()
    print('[SEED] ✔ Master data + inventory ready')


def _seed_moves_for_user(uid, moves_data, label):
    """Insert stock moves, lines, and history for a given user."""
    prods = {p.product_code: p for p in Product.query.all()}
    locs  = {l.short_code: l  for l in Location.query.all()}
    ...
    for ref, optype, contact_name, from_loc, to_loc, sdate, status, lines in moves_data:
        ...
        for pcode, qty in lines:
            db.session.add(StockMoveLine(
                move_id=m.id, product_id=prods[pcode].id, quantity=qty,
            ))

        if status == 'done':
            for pcode, qty in lines:
                db.session.add(MoveHistory(
                    move_id=m.id, product_id=prods[pcode].id,
                    from_location_id=locs[from_loc].id if from_loc else None,
                    to_location_id=locs[to_loc].id if to_loc else None,
                    quantity=qty,
                ))
    ...
    db.session.commit()
    print(f'[SEED] ✔ Moves seeded for {label}')


def _seed_stock_moves():
    """Seed stock moves for User 4 (Maithil) and User 5 (Myth12)."""
    ...
    _seed_moves_for_user(4, moves_u4, 'User 4 (Maithil)')
    _seed_moves_for_user(5, moves_u5, 'User 5 (Myth12)')
...
with app.app_context():
    try:
        ...
        # Seed master data if warehouses are empty
        if Warehouse.query.count() == 0:
            _seed_master_data()

        # Seed stock moves if none exist
        if StockMove.query.count() == 0:
            _seed_stock_moves()
            print('[SEED] 🎉 All seed data inserted automatically!')
